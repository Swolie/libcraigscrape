...
class CraigScrape::Posting < CraigScrape::Scraper
  ...
  JS_IMAGES       = /(http:\/\/.+?.jpg)/
  ADDRESS         = /<!-- CLTAG xstreet0=(.*?)\s*-->[\s.]*.*[\s]*.*city=(.*)\s*-->[\s.]*.*region=(.*)\s*-->/
  ...
  def address
    unless @address
      @address = "#{$1}, #{$2}, #{$3}" if ADDRESS.match html_source.to_s
    else
      @address
    end
  end
  ...
  def images
    ...
    if contents
      imgs = contents.scan(JS_IMAGES).collect{ |a| a.find{|b| !b.nil? } }
      imgs.shift
      imgs.each { |i| @images << i }
    end
    @images
  end
  ...
  def pics
    unless @pics
      ...
      if html and craigslist_body
        ...
        # img_table = (craigslist_body.css 'table').find{|e| e.name == 'table' and e[:summary] == 'craigslist hosted images'}
        # img_div = craigslist_body.at_css '.iw'
        # @pics = (img_table.css 'img').collect{|i| i[:src]} if img_table

        found = (craigslist_body.css 'img')
        @pics = found.collect{|i| i[:src]} unless found.blank?
      end
    end
  ...
  def html_head
    @html_head = Hpricot.parse $1 if @html_head.nil? and HTML_HEADER.match html_source.to_s
  ...
  def html_footer     
    $4 if USERBODY_PARTS.match html_source.to_s
  end
  ...
  def user_body     
    $2 if USERBODY_PARTS.match html_source.to_s
    # html_source.at_css("#userbody").to_s
  end
  ...
  def craigslist_body
    # html_source.at_css("#userbody").to_s
    # Hpricot.parse $3 if USERBODY_PARTS.match html_source.to_s
    Nokogiri.parse $3 if USERBODY_PARTS.match html_source.to_s
  end
